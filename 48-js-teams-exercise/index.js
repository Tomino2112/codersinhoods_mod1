 ...
 */
 ...
 //                                             MAKE LIST IN POOL 
  ...
  goToTeam1Btn.forEach(player =>{player.addEventListener("click", (event)=>{
    ...
    const team1List = document.querySelector('.column.team1');
    const team2List = document.querySelector('.column.team2');
    ...
    if (btnClasses.includes('to_team1')){
      ...
      createTeamMember(playerRemoved[0],team1List);

   
    } else if(btnClasses.includes('to_team2')){
      team2.push(playerRemoved[0]);
      createTeamMember(playerRemoved[0],team2List);
    }
  })
});
